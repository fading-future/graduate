...
def absolute_permeability_openpnm(
    ...
    # Network extraction path A (preferred):
    # snow_partitioning + regions_to_network
    # This avoids snow2's chunk-overlap parallelization warning in many cases.
    net_dict = None
    errs = []
    try:
        if (
            hasattr(ps, "filters")
            and hasattr(ps.filters, "snow_partitioning")
            and hasattr(ps, "networks")
            and hasattr(ps.networks, "regions_to_network")
        ):
            ...
            if regions is not None:
                net_candidate = ps.networks.regions_to_network(
                    regions=regions,
                    phases=im.astype(np.uint8),
                    voxel_size=1,
                    accuracy="standard",
                )
                net_dict = _unwrap_network(net_candidate)
    except Exception as e:
        errs.append(f"snow_partitioning+regions_to_network failed: {e}")

    # Network extraction path B (fallback): snow2
    if net_dict is None:
        try:
            if hasattr(ps, "networks") and hasattr(ps.networks, "snow2"):
                ...
                net_dict = _unwrap_network(snow)
        except Exception as e:
            errs.append(f"snow2 failed: {e}")

    if net_dict is None:
        msg = "Could not obtain network dictionary from PoreSpy"
        if errs:
            msg += " | " + " | ".join(errs)
